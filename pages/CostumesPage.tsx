...
const ShowcaseCard: React.FC<{ item: Costume; onInterestClick: () => void; }> = ({ item, onInterestClick }) => (
  <div className="bg-brand-gray rounded-lg overflow-hidden shadow-lg flex flex-col transform hover:-translate-y-1 transition-transform duration-300 group">
    ...
    <div className="p-4 flex flex-col flex-grow">
      <div className="flex-grow">
          ...
          <p className="text-gray-500 text-xs mt-2">Créateur: {item.creator}</p>
      </div>
...
const CostumesPage: React.FC = () => {
    ...
    const [interestedItem, setInterestedItem] = useState<Costume | null>(null);
    
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsLoading(true);
                const data = await apiClient.get('/api/costumes');
                setItems(data);
            } catch (err: any) {
    ...
    return (
        <div>
            ...
            <div className="bg-brand-gray p-6 rounded-lg mb-8 shadow-lg">
                <h1 className="text-3xl font-bold mb-4 text-center">Annuaire des Costumes</h1>
                ...
                <>
                    ...
                    {filteredItems.length === 0 && (
                        <div className="text-center py-16 text-gray-500">
                            <p className="text-2xl">Aucun costume trouvé.</p>
